...
export default function ActualitesAdminPage() {
  ...
  // ── Handlers via le modal de confirmation ──
  const handleDeleteClick = (actu) => {
    confirmModal.open({
      title: "Supprimer cet article ?",
      description: `"${actu.titre}" sera définitivement supprimé. Cette action est irréversible.`,
      variant: "danger",
      confirmLabel: "Supprimer",
      onConfirm: () => deleteActu(actu.id),
    });
  };

  const handleToggleClick = (actu) => {
    if (actu.publie) {
      confirmModal.open({
        title: "Mettre en brouillon ?",
        description: `"${actu.titre}" ne sera plus visible publiquement.`,
        variant: "warning",
        confirmLabel: "Mettre en brouillon",
        onConfirm: () => togglePublier(actu),
      });
    } else {
      confirmModal.open({
        title: "Publier cet article ?",
        description: `"${actu.titre}" sera visible par tous les visiteurs.`,
        variant: "info",
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <p className="text-gray-400 font-[Open_Sans] text-sm">
            {actualites.length} article(s) au total
          </p>
          <button
            onClick={() => openModal()}
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400 font-[Open_Sans]">
              ...
              >
                <div className="w-16 h-16 rounded-xl bg-gray-100 flex items-center justify-center shrink-0 overflow-hidden">
                  {a.image ? (
                    ...
                    <Newspaper size={20} className="text-[#0054A6]/40" />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-[Montserrat] font-bold text-gray-900 text-sm truncate">
                      {a.titre}
                    </h3>
                    <span
                      className={cn(
                        "shrink-0 text-xs font-[Open_Sans] font-semibold px-2 py-0.5 rounded-full",
                        a.publie
                  ...
                  <p className="text-gray-400 text-xs font-[Open_Sans]">
                    {formatDate(a.createdAt)}
                  </p>
                  <p className="text-gray-500 text-xs font-[Open_Sans] mt-1 line-clamp-1">
                    {a.extrait}
                  </p>
                </div>
                <div className="flex gap-2 shrink-0">
                  ...
                  <button
                    onClick={() => openModal(a)}
                    className="w-8 h-8 rounded-lg bg-gray-100 text-[#0054A6] flex items-center justify-center hover:bg-[#0054A6] hover:text-white transition-colors"
                  >
                    <Edit2 size={14} />
                  </button>
                  <button
            ...
            >
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5">
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  />
                  <span className="text-sm font-[Open_Sans] text-gray-700">
